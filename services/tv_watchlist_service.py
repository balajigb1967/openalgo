...
def _get_exchange_hint(token: str) -> str | None:
    """Determine exchange hint from TradingView token prefix.
    
    Handles special cases like TVC:FOREX -> FOREX, TVC:CRYPTO -> CRYPTO,
    then general cases like TVC:* -> MCX, GLOBAL_INDEX:* -> GLOBAL_INDEX.
    """
    token = token.strip().upper()
    
    # Handle specific known prefixes first
    if token.startswith("TVC:FOREX:"):
        return "FOREX"
    elif token.startswith("TVC:CRYPTO:"):
        return "CRYPTO"
    elif token == "TVC:FOREX" or token == "TVC:CRYPTO":
        # Handle cases where there's no symbol after the prefix
        return TV_EXCHANGE_MAP.get(token)
    
    # Handle general prefix cases
    if ":" in token:
        prefix, _, _ = token.partition(":")
        if prefix == "TVC":
            # Commodities: TVC:* maps to MCX
            return "MCX"
        elif prefix == "GLOBAL_INDEX":
            # Global indices: GLOBAL_INDEX:* maps to GLOBAL_INDEX
            return "GLOBAL_INDEX"
    
    # Handle dot notation (e.g., "NSE.RELIANCE")
...
#: TradingView exchange -> OpenAlgo exchange mapping for exact matches
TV_EXCHANGE_MAP = {
    "TVC:FOREX": "FOREX",
    "TVC:CRYPTO": "CRYPTO",
}
...
def resolve_symbol(token: str) -> tuple[str, str] | None:
    ...
    if ":" in token:
        ...
        exchange_hint = _get_exchange_hint(prefix + ":")
        token = rest.strip()
    elif "." in token and not token.split(".", 1)[1].isdigit():
