...
## FNO fork — agent playbook (balajigb1967 fork only)

The sections above describe upstream OpenAlgo as maintained by marketcalls.
This fork adds one layer on top of them — **`AGENTS.md`** at the repo root is
the cross-agent playbook for the FCC coding agents (claude, codex, opencode,
grok, dsh, pi). It covers the fork-specific facts upstream cannot know:

- **Branch discipline.** `main` mirrors upstream (fast-forward only, never
  commit); the customization line is `fno-capture` (plus `feat/*` branches).
- **FNO-custom map** — Fyers FNO symbol auto-resolution, illiquid-contract
  history messaging, TV-watchlist plugin, Market Depth panel, FOREX constant.
- **Known data reality** — Fyers serves no intraday for zero-volume contracts;
  MCX near-months move, so active contracts resolve dynamically.
- **MCP servers** — `openalgo` (upstream `mcp/mcpserver.py` trading + market
  data, launched via `scripts/oa_mcp_launch.sh`, API key read from the app DB)
  and `openalgo-ops` (`mcp_openalgo_server.py`: status/git/logs/health/history).

Read `AGENTS.md` first; where it is silent, the upstream guidance above rules.
